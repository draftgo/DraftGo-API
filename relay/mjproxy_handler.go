...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	if midjResponse.Code == 3 {
		...
		if channel.GetAutoBan() && common.AutomaticDisableChannelEnabled {
			reason := "No available account instance"
			if model.UpdateChannelStatus(midjourneyTask.ChannelId, "", common.ChannelStatusAutoDisabled, reason) {
				model.RecordChannelAutoDisableAuditLog(midjourneyTask.ChannelId, "", reason, channel.ChannelInfo.IsMultiKey)
			}
		}
	}
