...
func testChannel(ctx context.Context, channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool, timeoutSeconds int, options ...channelTestOptions) testResult {
	if ctx == nil {
...
func TestChannel(c *gin.Context) {
	...
	result := testChannel(requestCtx, channel, testUserID, testModel, endpointType, isStream, timeoutSeconds, testOptions)
...
func performChannelTests(ctx context.Context, channels []*model.Channel, testUserID int, allowDisable bool, report func(processed, total int)) channelTestSummary {
	...
	for index, channel := range channels {
		...
		tik := time.Now()
		result := testChannel(ctx, channel, testUserID, "", "", shouldUseStreamForAutomaticChannelTest(channel), common.ChannelTestDefaultTimeout)
		tok := time.Now()
		milliseconds := tok.Sub(tik).Milliseconds()
		if ctx != nil && ctx.Err() != nil {
			break
		...
		// enable channel
		if result.localErr == nil && !isChannelEnabled && shouldEnableChannelAfterRecoveryProbe(newAPIError, channel.Status, milliseconds) {
